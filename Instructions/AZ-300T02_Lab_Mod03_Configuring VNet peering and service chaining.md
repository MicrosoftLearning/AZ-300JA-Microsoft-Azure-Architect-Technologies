...
1. **az3000401-vnet**ブレードから、次の設定で VNet ピアリングを作成します。
    ...
    - 最初の仮想ネットワークから二番目の仮想ネットワークへの仮想ネットワーク アクセスを許可します。**有効にする**
...
1. リモート デスクトップ セッションで az3000401-vm2 に接続したら、**リモート アクセス** ロールをインストールします。サーバー マネージャーで、「**管理**」 をクリックし、「**役割と機能の追加**」 をクリックします。

1. 「開始する前に」画面で、「**次へ**」 をクリックします。

1. インストールの種類画面で、既定の 「**ロール ベースまたは機能ベースのインストール**」 を選択したままにして、「**次へ**」 をクリックします。

1. サーバーの選択画面で、既定のサーバーを選択したままにし、「**次へ**」 をクリックします

1. サーバーのロール画面で、「**リモート アクセス**」 の横にチェックマークを付けて、「**次へ**」 をクリックします。

1. 機能画面で、既定値を選択したままにして、「**次へ**」 をクリックします。

1. リモート アクセス画面で、「**次へ**」 をクリックします。

1. サービス画面で、「**ルーティング**」 の横にチェックマークを付けて、「**次へ**」 をクリックします。

1. 確認画面で、「**次へ**」 をクリックします。

1. 結果画面で、「**インストール**」 をクリックします。
...
1. 「**ルーティングとリモート アクセス**」 サービスを開始します。

1. az3000401-vm2 へのリモート デスクトップ セッションで、「スタート」 をクリックし、「**Windows 管理ツール**」 をクリックします。

1. 管理ツールから、「**セキュリティが強化された Windows ファイアウォール**」 コンソールを起動します。

1. コンソールで、「**受信規則**」 をクリックします。
...
1. az3000401-vm1 へのリモート デスクトップ セッションで、**詳細なセキュリティ コンソールを使用して Windows ファイアウォール**を起動し、 すべてのプロファイルに対して **ファイルとプリンタの共有 (エコー要求 - ICMPv4-In)** の受信ルールを有効にします。
